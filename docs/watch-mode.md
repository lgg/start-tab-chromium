...
Missing recursive static roots remain recoverable without restarting watch mode. Every required static input is validated before finalization. Explicit assets such as `manifest.json`, popup/options/blocked HTML and CSS, shared CSS, and full-profile new-tab assets must be real regular files. Recursive roots `icons/` and `src/_locales/` must be real directories containing only regular files/directories. Missing explicit files or recursive roots are reported as structured build errors while their containing directories remain watched, so restoring them can recover the same watch process without an unrelated edit. Links, junctions, and other special filesystem entries are rejected without traversal; wrong-type roots are rejected by the same structured contract. The same validation runs during ordinary one-shot/release builds that do not register the watch plugin.

Static output writes are serialized and guarded individually. After the whole-tree preflight, finalization revalidates each concrete static source immediately before its `cp()` or manifest read, then revalidates the selected build output immediately before the corresponding write. This narrows both the source-side and destination-side validation windows. Serial copy batches also ensure that a failed copy is not reported to lifecycle cleanup while a sibling `cp()` is still running; no late writer can recreate generated output after failure cleanup completes.

esbuild keeps JavaScript bundle outputs in memory (`write: false`) instead of writing them directly between lifecycle `onStart` and `onEnd`. After compilation succeeds, the lifecycle validates the production graph and static-input preflight, rechecks the output filesystem, requires the in-memory bundle set to match the exact profile-owned bundle paths, and writes each bundle sequentially through the same guarded output writer. Missing or unexpected chunks/maps therefore fail finalization before bundle materialization, and a late output junction/symlink cannot receive an unguarded JavaScript bundle.
